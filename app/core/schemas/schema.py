...
class ClfLabels(BaseModel):
    ground_truth: Union[List[int], List[List[int]]] = Field(examples=[[1, 2, 3, 3, 5]])
    predictions: Union[List[int], List[List[int]]] = Field(examples=[[1, 2, 3, 4, 5]])
...
class StabilityFileData(BaseModel):
    x_input: List[List[float]] = Field(examples=[[[0.46582937, 0.36313128, 0.17189367, 0.01546506],
    ...
    contributions: List[List[float]] = Field(examples=[[[0.40302247, 0.2380319, 0.2301524, 0.51886267],
...
class StabilityData(BaseModel):
    x_train: List[List[float]] = Field(examples=[[[0.24763825, 0.4624466, 0.1439733, 0.63356432],
    ...
    y_train: List[List[float]] = Field(examples=[[[1], [1], [1], [1], [0]]])
    # Contributions from one type of explainer
    x_input: List[List[float]] = Field(examples=[[[0.46582937, 0.36313128, 0.17189367, 0.01546506],
                                                  [0.507121, 0.01918931, 0.78464877, 0.77427306],
                                                  [0.50496966, 0.98061098, 0.27967449, 0.29417485],
...
class UserDiversityInput(BaseModel):
    predictions: List[List[float]] = Field(examples=[[[-2.218350887298584, -2.198277711868286],
    ...
    client_ids: List[int] = Field(examples=[[1, 1, 1, 2, 2, 2]])
    perplexity: int = Field(examples=[2])
