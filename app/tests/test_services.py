...
from app.services.metrics_service import consistency_scores, consistency_plot, \
    compacity_plot, stability_plot, user_diversity_plot, metric_utils
import io
import joblib
...
def test_service_stability_with_received_model_plot():
    ...
    with open("assets/test_stability_received_model_buffered_image.png", "wb") as f:
        f.write(stabl_plot.getvalue())
    assert isinstance(stabl_plot, io.BytesIO)


def test_service_stability_with_surrogate_model_training_plot():
    # Basic invocation test
    selection = None
    max_points = 500
    max_features = 2
