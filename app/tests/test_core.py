...
def test_shapash_stability_with_sent_model_plot():
    # Test the output of metrics.shapash_compacity_plot
    ...
    img_buf = io.BytesIO()
    img_bytes = stability_plot.to_image('png', engine='kaleido')
    stability_plot.write_image("assets/test_stability_core_with_sent_model_image.png", format="png")
    stability_plot.write_image(img_buf, format="png")
    assert isinstance(img_buf, io.BytesIO)
    assert isinstance(img_bytes, bytes)
...
def test_shapash_stability_with_surrogate_model_training_plot():
    ...
    img_buf = io.BytesIO()
    img_bytes = stability_plot.to_image('png', engine='kaleido')
    stability_plot.write_image("assets/test_stability_core_with_surrogate_model_training_image.png", format="png")
    stability_plot.write_image(img_buf, format="png")
    assert isinstance(img_buf, io.BytesIO)
    assert isinstance(img_bytes, bytes)
...
def test_shapash_stability():
    # TODO: Development is not complete
    # Test the output of metrics.shapash_compacity_plot
    selection = None
    max_points = 500
    max_features = 2
